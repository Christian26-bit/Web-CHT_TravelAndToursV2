...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function searchClients(query) {
    try {
      const response = await fetch(BASE_URL + `api/clients_list?q=${encodeURIComponent(query)}`);
    ...
    nextBtn.addEventListener('click', async () => {
      ...
      if (!selectedClientId) {
        try {
          ...
          formData.append('type', travelTypeField.value.trim() || 'Leisure');

          const response = await fetch(BASE_URL + 'api/clients_save', {
            method: 'POST',
            body: formData
          });
