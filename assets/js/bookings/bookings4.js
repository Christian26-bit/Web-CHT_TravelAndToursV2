...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadHotels() {
    try {
      ...
      const response = await fetch(BASE_URL + 'api/hotels_list');
  ...
  function renderHotels(filterRating = "all") {
    ...
    hotelListInner.innerHTML = filtered.map(hotel => {
      ...
      return `
        <div class="hotel-row cv-card-clickable ${hotel.id === selectedHotelId ? 'selected' : ''}" data-id="${hotel.id}">
  ...
  if (nextBtn) {
    nextBtn.addEventListener('click', () => {
      ...
      window.location.href = BASE_URL + 'bookings/5';
    });
  }
