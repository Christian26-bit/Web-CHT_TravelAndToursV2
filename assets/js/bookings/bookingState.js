...
const BookingState = {
  ...
  calculateTotal() {
    const state = this.get();
    let total = 0;
    const pax = Number(state.customer.pax) || 1;
    
    // Package price (multiplied by pax count)
    total += (Number(state.package.price) || 0) * pax;
  ...
  populateSummary() {
    ...
    if (summaryBreakdown) {
      ...
      if (state.package.price) {
          const pkgTotal = state.package.price * pax;
          breakdown.push(`Package: ${this.formatPrice(pkgTotal)} (${pax} pax)`);
      }
      if (state.addons.totalPrice) breakdown.push(`Add-ons: ${this.formatPrice(state.addons.totalPrice)}`);
      if (state.hotel.pricePerNight) breakdown.push(`Hotel: ${this.formatPrice(state.hotel.pricePerNight * duration)}`);
      if (state.transport.pricePerDay) breakdown.push(`Transport: ${this.formatPrice(state.transport.pricePerDay * duration)}`);
  ...
  if (newBookingBtn) {
    newBookingBtn.addEventListener('click', () => {
      if (BookingState.hasActiveBooking()) {
        ...
        if (confirmNew) {
          BookingState.clear();
          window.location.href = window.location.pathname.includes('Bookings') ? 'bookings/1' : 'bookings/1';
        }
      } else {
        BookingState.clear();
        window.location.href = window.location.pathname.includes('Bookings') ? 'bookings/1' : 'bookings/1';
