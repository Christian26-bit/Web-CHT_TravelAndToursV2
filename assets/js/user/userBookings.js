...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadBookings(searchQuery = '') {
    try {
      ...
      const url = searchQuery 
        ? BASE_URL + `api/bookings_list?q=${encodeURIComponent(searchQuery)}`
        : BASE_URL + 'api/bookings_list';
      ...
      bookingsTableBody.innerHTML = `
        <tr>
          <td colspan="8" class="no-data">
            No bookings found. <a href="bookings/1">Create your first booking</a>
          </td>
        </tr>
      `;
  ...
  if (newBookingBtnTop) {
    newBookingBtnTop.addEventListener('click', () => {
      ...
      window.location.href = BASE_URL + 'bookings/1';
    });
  }
    ...
    bookingsTableBody.addEventListener('click', async (e) => {
      ...
      if (btn.classList.contains('btn-view')) {
        ...
        if (confirm(`Are you sure you want to delete booking ${booking.ref}? This cannot be undone.`)) {
          try {
            const response = await fetch(BASE_URL + 'api/bookings_delete', {
              method: 'POST',
  ...
  async function populatePackageAndDestinationDropdowns(selectedPackage, selectedDestination) {
    ...
    try {
      const res = await fetch(BASE_URL + 'api/packages_list');
  ...
  if (editBookingForm) {
    editBookingForm.onsubmit = async function(e) {
      ...
      try {
        const response = await fetch(BASE_URL + 'api/bookings_update', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
