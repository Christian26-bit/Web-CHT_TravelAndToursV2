...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadVehicles() {
    try {
      ...
      const response = await fetch(BASE_URL + 'api/transportation_list');
  ...
  function renderVehicles(filterType = "all") {
    ...
    transportListInner.innerHTML = filtered.map(vehicle => `
      <div class="transport-row cv-card-clickable ${vehicle.id === selectedTransportId ? 'selected' : ''}" data-id="${vehicle.id}">
  ...
  if (nextBtn) {
    nextBtn.addEventListener('click', () => {
      ...
      window.location.href = BASE_URL + 'bookings/6';
    });
  }
