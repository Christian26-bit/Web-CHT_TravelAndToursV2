...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadPackages() {
    try {
      ...
      const response = await fetch(BASE_URL + 'api/packages_list');
  ...
  function renderPackages() {
    packageGrid.innerHTML = packages.map(pkg => `
      <article class="package-card cv-card-clickable ${pkg.id === selectedPackageId ? 'selected' : ''}" data-id="${pkg.id}">
  ...
  if (nextBtn) {
    nextBtn.addEventListener('click', () => {
      ...
      window.location.href = BASE_URL + 'bookings/3';
    });
  }
