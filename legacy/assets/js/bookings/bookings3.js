...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadAddons() {
    try {
      ...
      const response = await fetch(BASE_URL + 'api/addons_list');
  ...
  function renderAddons() {
    addonsListEl.innerHTML = addons.map(addon => {
      ...
      return `
        <div class="cv-card cv-card-clickable ${isChecked ? 'selected' : ''}" style="display: flex; align-items: center; justify-content: space-between; padding: 14px 20px; margin-bottom: 10px;" onclick="document.getElementById('addon-${addon.id}').click()">
          <div style="display: flex; align-items: center; gap: 16px;">
            <input type="checkbox" class="addon-checkbox" id="addon-${addon.id}" data-id="${addon.id}" ${isChecked ? 'checked' : ''} style="width: 20px; height: 20px; cursor: pointer;" onclick="event.stopPropagation()">
            <div>
              <div style="font-weight: 700; color: var(--text-dark); font-size: 14px;">${addon.name}</div>
              <div style="font-size: 12px; color: var(--text-muted);">${addon.description}</div>
            </div>
          </div>
          <div style="font-weight: 800; color: var(--green); font-size: 14px;">+₱${Number(addon.price).toLocaleString()}</div>
        </div>
      `;
    }).join('');
  }
  ...
  if (backBtn) {
    backBtn.addEventListener('click', () => {
      window.location.href = BASE_URL + 'bookings/2';
    });
  }
  ...
  if (nextBtn) {
    nextBtn.addEventListener('click', () => {
      ...
      window.location.href = BASE_URL + 'bookings/4';
    });
  }
