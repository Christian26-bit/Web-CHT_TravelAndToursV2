document.addEventListener("DOMContentLoaded", () => {
 ...
 function loadPackages() {
  fetch(BASE_URL + "api/list_packages")
    .then(r => r.json())
    .then(data => {
  ...
  function setFormEdit(pkg) {
    ...
    resetImagePreview();
    if (pkg.image_path) {
      showImagePreview(BASE_URL + pkg.image_path);
    }
  }
  ...
  tableBody.addEventListener("click", e => {
    ...
    if (btn.dataset.action === "edit") {
      ...
      fd.append("id", id);
      fetch(BASE_URL + "api/delete_package", { method: "POST", body: fd })
        .then(r => r.json())
        .then(res => {
          if (res.success) {
            loadPackages();
  ...
  form.addEventListener("submit", e => {
    ...
    fetch(BASE_URL + "api/save_package", { method: "POST", body: fd })
      .then(r => r.json())
      .then(res => {
        if (res.success) {
          loadPackages();
